    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">Manage and view all financial transactions</p>
        </div>
        <div className="flex items-center space-x-2 sm:space-x-3">
          <button className="flex items-center space-x-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 px-3 sm:px-4 py-2 rounded-lg transition whitespace-nowrap font-medium shadow-sm">
            <RefreshCw size={18} />
            <span className="hidden sm:inline">Refresh</span>
          </button>
          <button className="flex items-center space-x-2 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg transition whitespace-nowrap shadow-md hover:shadow-lg font-medium">
            <Download size={18} />
            <span>Export</span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-5 shadow-sm hover:shadow-md transition">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="w-7 h-7 sm:w-8 sm:h-8 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center">
              <Wallet size={14} className="sm:w-4 sm:h-4 text-blue-600 dark:text-blue-400" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">${stats.total.toLocaleString()}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{stats.totalCount} txns</p>
        </div>

        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-5 shadow-sm hover:shadow-md transition">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Deposits</p>
          ...
          <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">${stats.deposits.toLocaleString()}</p>
          <p className="text-xs text-green-600 dark:text-green-400 mt-1 font-semibold">Incoming</p>
        </div>

        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-5 shadow-sm hover:shadow-md transition">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Withdrawals</p>
            <div className="w-7 h-7 sm:w-8 sm:h-8 bg-red-100 dark:bg-red-900/30 rounded-lg flex items-center justify-center">
              <TrendingDown size={14} className="sm:w-4 sm:h-4 text-red-600 dark:text-red-400" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">${stats.withdrawals.toLocaleString()}</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-5 shadow-sm hover:shadow-md transition">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {((stats.completed / stats.totalCount) * 100).toFixed(1)}% success
          </p>
        </div>
      </div>
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-cyan-500 flex items-center justify-center text-white font-semibold text-sm">
                  {hiddenRows.has(transaction.id) ? '?' : transaction.user.charAt(0).toUpperCase()}
                </div>
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white">
                    {hiddenRows.has(transaction.id) ? '•••••••' : transaction.user}
                  </p>
                  <p className="text-xs font-mono text-gray-500 dark:text-gray-400">
                    {hiddenRows.has(transaction.id) ? '••••••' : transaction.id}
                  </p>
                </div>
              </div>
              <button 
                onClick={() => toggleRowVisibility(transaction.id)}
                className={`p-2 rounded-lg transition ${
                  hiddenRows.has(transaction.id) 
                    ? 'text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700' 
                    : 'text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30'
                }`}
              >
                {hiddenRows.has(transaction.id) ? <EyeOff size={18} /> : <Eye size={18} />}
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <p className="text-lg font-bold text-gray-900 dark:text-white">
                  {hiddenRows.has(transaction.id) ? '$•••••' : `$${transaction.amount.toLocaleString()}`}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Date</p>
                <p className="text-sm text-gray-700 dark:text-gray-300">
                  {hiddenRows.has(transaction.id) ? '••/••/••••' : transaction.date}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-gray-700">
              <div className="flex items-center space-x-2">
                {!hiddenRows.has(transaction.id) && (
                  <>
                    {columns[3].render(transaction.type, transaction)}
                    {columns[4].render(transaction.status, transaction)}
                  </>
                )}
                {hiddenRows.has(transaction.id) && (
                  <>
